...
export class ProgramService implements IProgramService {
  ...
  async getProgramWall(
    ...
    if (program_classification) {
      // 添加 where condition
      handle = handle.andWhere('program_classification = :program_classification', { program_classification });
    }

    if (release_year) {
      // 添加 where condition
      handle = handle.andWhere('release_year = :release_year', { release_year });
    }

    if (program_area) {
      handle = handle.andWhere('program_area = :program_area', { program_area });
    }
